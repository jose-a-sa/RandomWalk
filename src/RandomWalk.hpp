...
public:
    ...
    std::vector<std::size_t> walkWhileSample(std::size_t sample_size, std::function<bool(Point<int, D>)> boundary, std::size_t max_iter = -1);
};
...
template <std::size_t D>
inline RandomWalk<D>::RandomWalk(const Point<Point<std::size_t, 2>, D> &weights, const Point<int, D> &dxx) : dx(std::move(dxx))
{
    init_rand_();
